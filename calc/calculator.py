...
def select_pipe(input_data):
    ...
    actual_flow = (flow_rate/(1000*60))*(101.3/(101.3+outlet_pressure*1000))*((273+temperature)/273)
    required_diameter = math.sqrt((4 * actual_flow) / (math.pi * velocity_limit)) * 1000

    pipe_table = pd.read_csv(PIPE_FILES[schedule])

    filtered = pipe_table[
        pipe_table["内径(D)(mm)"] >= required_diameter
    ]

    for _, row in filtered.iterrows():
        fluid_density = ((inlet_pressure*10.1972+1.033)/1.033*101325)/(8314.3*293/M)
        n = sum([fitting_counts[i] * fitting_equivalent_lengths[i] for i in range(len(fitting_counts))])
        Ln = row["内径(D)(mm)"] * n / 1000
        delta_P = (((row["摩擦係数"])*(pipe_length+Ln)*fluid_density)*(velocity_limit**2))/(row["内径(D)(mm)"]*0.001*2*9.8)*0.0001
